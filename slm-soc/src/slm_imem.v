/*
 * slm_imem — dual-port instruction memory.
 *
 * Port A: private CPU fetch port — synchronous read, 1-cycle latency,
 * always ready. Port B: SLB slave port with per-byte write strobes so the
 * program loader (CPU/DMA) can write and read the array. The storage array
 * is named `mem` (testbenches load programs via hierarchical references)
 * and is zero-initialized in an initial block (synthesis-benign for
 * behavioral RAM; keeps simulation X-free). Both ports are word-addressed:
 * byte address bits [1:0] are ignored, and addresses beyond WORDS alias
 * (the xbar decode guarantees an 8 KiB window in sim).
 *
...
module slm_imem #(
  ...
  input  wire [31:0] if_addr,      // port A: fetch byte address (word-aligned)
  output reg  [31:0] if_rdata,     // port A: instruction, valid next cycle
  // port B: SLB slave
  ...
  wire [29:0]   if_word = if_addr[31:2];
  wire [13:0]   s_word  = s_req_addr[15:2];
  wire [AW-1:0] if_idx  = if_word[AW-1:0];
  wire [AW-1:0] s_idx   = s_word[AW-1:0];

  // port A: fetch (sync read, always ready)
  always @(posedge clk) begin
    if (!rst_n)
      if_rdata <= 32'h0000_0000;
    else
      if_rdata <= mem[if_idx];
  end

  // port B: SLB slave, response on the cycle after acceptance
  ...
  wire _unused = &{1'b0, if_addr[1:0], s_req_addr[1:0], if_word, s_word};
