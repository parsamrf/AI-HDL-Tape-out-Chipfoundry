/*
 * TinyTapeout tile: fixed-point softmax accelerator from an SLM inference SoC.
 * SPDX-License-Identifier: Apache-2.0
 *
 * The block's real CSR interface is exposed through a 32-bit SPI register
 * bridge, so the tile is fully exercisable stand-alone — and the same SLB
 * port could be bus-connected if inter-tile wiring is available.
 *
 * Pins (same convention as the other bridge tiles):
 *   uio[4] CS_n (in) · uio[5] SCK (in) · uio[6] MOSI (in) · uio[3] MISO (out)
 *   uio[0] irq (out) — softmax done (when enabled in CFG)
 */
`default_nettype none

module tt_um_slm_softmax (
